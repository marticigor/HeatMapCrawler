...
public class BaseFilter implements I_ImageProcesor {
	...
	protected BaseFilter(
			...
			int x, int y, int border, boolean wholeImage, boolean debug, int expectedArgsLength, int[] args
		...
		if (args.length != expectedArgsLength)
			throw new RuntimeException("Arguments length in BaseFilter");
		this.args = args;
		borderSharpenStage = args[4];
		if (debug)
			for (int i : args)
				System.out.println("args in BaseFilter: " + i);
		...
		if (halt)
			throw new RuntimeException("chunks messed");
		...
		if (borderSharpenStage < border)
			throw new RuntimeException("kernel mess BaseFilter");
	}
