	...
	public void doYourThing() {
		...
		for (int absX = widthFrom; absX < widthTo; absX++) {
			for (int absY = heightFrom; absY < heightTo; absY++) {
				...
				/*
				 * IF TO SLOW ... A Gaussian blur effect is typically generated by convolving an
				 * image with a kernel of Gaussian values. In practice, it is best to take
				 * advantage of the Gaussian blur’s separable property by dividing the process
				 * into two passes. In the first pass, a one-dimensional kernel is used to blur
				 * the image in only the horizontal or vertical direction. In the second pass,
				 * the same one-dimensional kernel is used to blur in the remaining direction.
				...
				r = (int) ((cumulR) * Gaussian.NORMALIZE_GAUS_5);
				g = (int) ((cumulG) * Gaussian.NORMALIZE_GAUS_5);
				b = (int) ((cumulB) * Gaussian.NORMALIZE_GAUS_5);
