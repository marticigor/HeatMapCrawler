    ...
    if (isSetToClusterAround(pOfNoded)) { //2 white check only
     ...
     out: {
       ...
       for (Node maybeClosest: maximusNodes) {
        ...
        /**
        //else keep track of closest X and Y value not necesarrily existing node!!
        if (Math.abs(maybeClosest.getX() - centerGravityX) < closestXsoFar) {
         closestXsoFar = Math.abs(maybeClosest.getX() - centerGravityX);
         closestToCenterGravityX = maybeClosest.getX();
        }
        if (Math.abs(maybeClosest.getY() - centerGravityY) < closestYsoFar) {
         closestYsoFar = Math.abs(maybeClosest.getY() - centerGravityY);
         closestToCenterGravityX = maybeClosest.getY();
        }
        */
       ...
       /**
         ...
         * testing code
         *
        Pixel pixelBlue = noded.getPixel(closestToCenterGravityX,closestToCenterGravityX);
        pixelBlue.setRed(0);
        pixelBlue.setGreen(100);
        pixelBlue.setBlue(255);
        noded.draw();
        
       //Pixel pix = noded.getPixel(closestToCenterGravityX, closestToCenterGravityX);
       //boolean white = isSetToClusterAround(pix);
