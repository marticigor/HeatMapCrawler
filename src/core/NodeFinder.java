...
public class NodeFinder implements I_ColorScheme {
	...
	public void findNodes() {
		...
		for (Pixel pOfNoded : noded.pixels()) { // 1
			...
			if (isSetToClusterAround(pOfNoded)) { // 2 white check only
				...
				setClustered(pOfNoded); // as this emerges from recursion we
										// have filled allClusterAroundNode
				...
				out: {
					...
					for (Node maybeClosest : maximusNodes) {
						...
						// calculate dist to center of gravity and set it to
						// node POJO
	...
	/**
	 * loads recursively set of white pixels into neighbours and then
	 * allClusterAroundNode
	 */
