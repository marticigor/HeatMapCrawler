...
public class DistanceMapSkeleton extends BaseFilter implements I_ColorScheme {
	...
	private void computeDistanceMap() {
		...
		for (Pixel pivot : in.pixels()) {
			if (utils.isForeground(pivot)) {
				...
				if (green > maxDistComputed)
					maxDistComputed = green;
	...
	private int getDistToClosestFeaturePixel(Pixel pivot) {
		...
		while (i < searchSpan) {
			...
			halfI = i / 2;
			c1 = in.getPixel(utils.clipXToImageBounds(pivot.getX() - halfI),
					utils.clipYToImageBounds(pivot.getY() - halfI));
			c2 = in.getPixel(utils.clipXToImageBounds(pivot.getX() + halfI),
					utils.clipYToImageBounds(pivot.getY() - halfI));
			c3 = in.getPixel(utils.clipXToImageBounds(pivot.getX() + halfI),
					utils.clipYToImageBounds(pivot.getY() + halfI));
			c4 = in.getPixel(utils.clipXToImageBounds(pivot.getX() - halfI),
					utils.clipYToImageBounds(pivot.getY() + halfI));
		...
		for (Integer dist : distances)
			if (dist < minCartesianDist)
				minCartesianDist = dist;
		return minCartesianDist;
	}
